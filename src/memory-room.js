...
export function createMemoryRoom(exterior){
 const scene=new THREE.Scene();scene.background=new THREE.Color('#e9dfcc');
 ...
 scene.add(new THREE.HemisphereLight('#fff5dd','#b4a98d',1.55));
 const sun=new THREE.DirectionalLight('#ffe4ae',3.0);sun.position.set(2.5,7,-1.7);sun.target.position.set(-1,0,2.4);scene.add(sun,sun.target);
 ...
 const warmLamp=new THREE.PointLight('#ffb873',4.0,7,2);warmLamp.position.set(-3.5,2.1,-.1);scene.add(warmLamp);
 const fill=new THREE.DirectionalLight('#f9efe1',.65);fill.position.set(4,3,9);scene.add(fill);
 ...
 // Layered plinth and varied narrow floor boards.
 round(10.35,.27,8.05,.10,0,-.13,.1,palette.edge);
 ...
 // Back wall is physically open at the sea-facing window.
 ...
 box(.2,4.8,8,-5.0,2.4,.1,palette.wall);
 // Sage beadboard, skirting and picture ledges.
 ...
 // A large live window, inset frame, sill, delicate mullions and curtains.
 ...
 // Rounded linen sofa, tailored piping, relaxed cushions and folded throw.
 ...
 // Window seat with tufted linen pad and woven baskets underneath.
 ...
 // Layered oval rug with braided edge, lightly fringed ends.
 ...
 // Oval coffee table, two mugs, a little vase and a handwritten envelope.
 ...
 // A reading armchair, lamp and softly patterned throw pillow.
 ...
 // Low book cabinet to the right, scalloped pottery, a keepsake frame.
 ...
 // A tall, spreading plant beside the window.
 ...
 // Warm fairy lights drape above the memory wall.
 ...
 // Combine static decorations by material to keep the detailed room inexpensive.
 ...
 return {
  update,windowView,
  pick(x,y){raycaster.setFromCamera(new THREE.Vector2(x,y),camera);const hit=raycaster.intersectObjects([...frames,paper,envelope])[0];return hit?hit.object.userData:null;},
  render(renderer,time){
   windowView.render(renderer,time);
   camera.aspect=innerWidth/innerHeight;const fit=Math.max(1,.84/camera.aspect);camera.position.set(8.9*fit,6.3*fit,12.2*fit);camera.lookAt(0,1.8,-.2);camera.updateProjectionMatrix();
