...
export function createWaterRefraction(renderer,scene,camera,ocean,uniforms){
 ...
 return {render(){
  ...
  if(!moved&&frame++%2===1){uniforms.uReefReady.value=1;return;}
  ...
  try{
   ...
   uniforms.uReefMatrix.value.multiplyMatrices(camera.projectionMatrix,camera.matrixWorldInverse);
 ...
 },finish(){uniforms.uReefReady.value=0;},dispose(){target.dispose();}};
}
